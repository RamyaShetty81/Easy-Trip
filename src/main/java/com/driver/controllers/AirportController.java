...
public class AirportController {
    ...
    public double getShortestDurationOfPossibleBetweenTwoCities(@RequestParam("fromCity") City fromCity,
        ...
        double short1 = Integer.MAX_VALUE;

        for(Flight flight:flightDb.values()){
            if(flight.getFromCity()==fromCity && flight.getToCity()==toCity){
                short1=Math.min(short1,flight.getDuration());
            }
        }

        if(short1 ==Integer.MAX_VALUE) {
            return -1;
        }
    ...
    public int calculateFlightFare(@RequestParam("flightId")Integer flightId){
        ...
        //Suppose if 2 people have booked the flight already : the price of flight for the
        // //third person will be 3000 + 2*50 = 3100
    ...
    public int countOfBookingsDoneByPassengerAllCombined(@PathVariable("passengerId")Integer passengerId){
        //Tell the count of flight bookings done by a passenger: This will tell
        // the total count of flight bookings done by a passenger :
    ...
    public int calculateRevenueOfAFlight(@PathVariable("flightId")Integer flightId){
        ...
        return (flightToPassengerDb.get(flightId).size() * (flightToPassengerDb.get(flightId).size() - 1))*25 +
                3000*flightToPassengerDb.get(flightId).size() ;
    }
